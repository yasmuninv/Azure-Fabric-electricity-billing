...
# Human behaviour (wake up, get home, solar noon) happens on the wall
# clock, i.e. LOCAL time -- daily_shape()/solar_offset() are defined in
# terms of it. reading_ts is still stored in UTC (see reading_for).
...
def seasonal_factor(month):
    # Single winter-peaked cosine: 1.4x in January, 0.6x in July, smooth
    # in between. Nordic residential load is heating-dominated with
    # minimal AC, so one winter peak is realistic as-is. If you want a
    # bimodal winter+summer (AC) curve instead, add a second harmonic,
    # e.g. `+ 0.15 * math.cos((month - 1) / 6.0 * 2 * math.pi)`.
...
def reading_for(meter, ts, rng):
    # ts is UTC internally (see stream_readings); shape/season logic is
    # about wall-clock human behaviour, so it must run on LOCAL time, or
    # every peak lands 1-2h late (CET/CEST is UTC+1/+2). reading_ts is
    # still stored in UTC below, which is correct -- only the *shape*
    # inputs need the local conversion.
    local_ts = ts.astimezone(LOCAL_TZ)
    hour = local_ts.hour + local_ts.minute / 60.0
    weekend_boost = 1.15 if local_ts.weekday() >= 5 else 1.0

    shape = daily_shape(hour) * meter["peak_factor"] * weekend_boost
    season = seasonal_factor(local_ts.month)
    ...
    return {
        ...
        # No tariff here on purpose: a real meter has no idea what
        # billing plan its customer is on. Tariff is resolved in Silver
        # via an as-of join against dim_meter_tariff (SCD2) -- see
        # reference_builder.py and silver_transform_databricks.py.
